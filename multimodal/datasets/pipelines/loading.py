...
@PIPELINES.register_module()
class LoadVideoFeature:
    """Load offline extracted video features.

    Required keys are "filename", added or modified keys are "num_segs",
    imgs".
    """
    ...
    def __init__(self, 
        ...
        if backend == 'disk':
            ...
            assert ann_file and suffix, \
                'require ann_file and data_root to get video features'
            self.id2feat = self.__memcache_dataset_feature(ann_file, data_prefix, suffix)
        else:
            raise NotImplementedError
    
    def __memcache_dataset_feature(self, ann_file, data_prefix, suffix):
        from tqdm import tqdm
        id2feat = {}
        with open(ann_file, 'r') as fin:
            lines = fin.readlines()
        for line in tqdm(lines, desc='Load dataset feature into RAM ...'):
            line_split = line.strip().split()
            filename = line_split[0]
            if filename.endswith('.mp4'):
                id = filename.split('/')[-1].split('.')[0]
                filename = filename[:-4]
                feat_path = osp.join(data_prefix,
                                    filename) + suffix
                ...
                id = filename.split('/')[-1].split('.')[0]
                feat_path = osp.join(data_prefix, filename)
            feature_map = self.__load_feature(feat_path)
            id2feat[id] = feature_map
        return id2feat
    
    def __load_feature(self, feat_path):
        if feat_path.endswith('.pkl'):
            with open(feat_path, 'rb') as F:
                feature_map = pickle.load(F)
        elif feat_path.endswith('.npy'):
            feature_map = np.load(feat_path)
        else:
            raise NotImplementedError
        # pooling
        if self.feat_level == 'video':
            assert len(feature_map.shape) == 1, \
                f'Got frame level feature with shape {feature_map.shape}, expected video level!'
        elif self.feat_level == 'frame':
            raise NotImplementedError
        else:
            raise NotImplementedError
        return feature_map
    ...
    def __call__(self, results):
        ...
        if self.backend == 'disk':
            feature_map = self.__load_feature(results['video_feat_path'])
        elif self.backend == 'memcached':
            id = results['video_feat_path'].split('/')[-1].split('.')[0]
            feature_map = self.id2feat[id]
        results['imgs'] = feature_map
        return results
...
@PIPELINES.register_module()
class LoadTextFeature:
    """Load offline extracted text features.
    ...
    def __call__(self, results):
        ...
        Args:
            results (dict): The resulting dict to be modified and passed
                to the next transform in pipeline.
        """
        if results['text_feat_path'].endswith('.pkl'):
            with open(results['text_feat_path'], 'rb') as F:
                feature_map = pickle.load(F)
        elif results['text_feat_path'].endswith('.npy'):
            feature_map = np.load(results['text_feat_path'])

        results['sents'] = feature_map
        return results
