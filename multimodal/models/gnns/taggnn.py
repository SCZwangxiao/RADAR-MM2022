...
class GAT(GATConv):
    def __init__(self,
        ...
        self.agg = nn.Linear(out_feats, out_feats, bias=False)
        self.gate_fc1 = nn.Linear(out_feats, out_feats, bias=False)
        self.gate_fc2 = nn.Linear(out_feats, out_feats)

    def aggregate_and_update(self, h0, hm):
        # Aggregate neighbour message and itself
        hm = hm.reshape(-1, self.h_dim)
        hm = self.agg(h0 + hm)
        hm = self.activation(hm)
        # Update
        z = self.gate_fc1(hm) + self.gate_fc2(h0)
        z = torch.sigmoid(z)
        h1 = z * hm + (1 - z) * h0
        return h1
...
class TagGNN(nn.Module):
    ...
    def forward(self, input_nodes, pair_graph, mfgs):
        ...
        for l in range(self.num_layers):
            ...
            if 'video' not in h:
                h['video'] = h_layer[-1]['video'][:mfg.num_dst_nodes('video')]
            h_layer.append(h)
