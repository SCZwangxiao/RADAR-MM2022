...
class TransformerMessagePasser(nn.Module):
    ...
    def followedby_message(self, sub_graph, h, h0, return_logits=False):
        ...
        attn_score_logits = attn_score
        attn_score = edge_softmax(sub_graph, attn_score, norm_by='dst')
        sub_graph.edata['t'] = attn_score.unsqueeze(-1)
        if return_logits:
            # attn_score_logits = torch.exp(attn_score_logits)
            attn_score_logits = attn_score
            return attn_score_logits
    
    def hasvideo_message(self, sub_graph, h, h0, return_logits=False):
        srctype, etype, dsttype = ('tag', 'HasVideo', 'video')
        e_id = self.etypes[etype]
        k_linear = self.k_linears[srctype]
        v_linear = self.v_linears[srctype]
        q_linear = self.q_linears[dsttype]
    ...
    def supertopic_message(self, sub_graph, h, h0):
        srctype, etype, dsttype = ('tag', 'SuperTopic', 'tag')
        e_id = self.etypes[etype]
        k_linear = self.k_linears[srctype]
        v_linear = self.v_linears[srctype]
        q_linear = self.q_linears[dsttype]
    ...
    def forward(self, mfg, h, h0):
        for relation in self.relations:
            ...
            if etype == 'FollowedBy':
                ...
                self.hasvideo_message(sub_graph, h, h0)
            elif etype == 'SubTopic':
                self.subtopic_message(sub_graph, h, h0)
            elif etype == 'SuperTopic':
                self.supertopic_message(sub_graph, h, h0)
            elif etype == 'HasTag':
...
class GatedUpdator(nn.Module):
    def __init__(self,
        ...
        self.skip_fc = nn.ModuleDict()
        self.skip_gate_fc1 = nn.ModuleDict()
        self.skip_gate_fc2 = nn.ModuleDict()
        self.drop = nn.Dropout(dropout)
        for dsttype in self.dst_ntypes:
            self.skip_fc[dsttype] = nn.Linear(hidden_dim, hidden_dim, bias=False)
            self.skip_gate_fc1[dsttype] = nn.Linear(hidden_dim, hidden_dim, bias=False)
            self.skip_gate_fc2[dsttype] = nn.Linear(hidden_dim, hidden_dim)
    ...
    def forward(self, mfg, h, h0):
        ...
        for ntype, n_id in self.dst_ntypes.items():
            skip_fc = self.skip_fc[ntype]
            skip_gate_fc1 = self.skip_gate_fc1[ntype]
            skip_gate_fc2 = self.skip_gate_fc2[ntype]
            hm = mfg.dstdata['hm'][ntype]
            n_intype = hm.shape[1]

            hm = mfg.dstdata['hm'][ntype].view(-1, n_intype, self.hidden_dim)
            h0_dst = h0[ntype].unsqueeze(1).expand(-1, n_intype, -1)
            z = skip_gate_fc1(hm) + skip_gate_fc2(h0_dst) # 
            hm = F.gelu(self.drop(skip_fc(h0_dst + hm))) # 
            z = torch.sigmoid(z)
            trans_out[ntype] = z * hm + (1 - z) * h0_dst
        return trans_out
    
    def get_delta_z(self, mfg, h, h0):
        # get_deltaz torch.zeros_like(hm)
        z_out = {}
        for ntype, n_id in self.dst_ntypes.items():
            skip_fc = self.skip_fc[ntype]
            skip_gate_fc1 = self.skip_gate_fc1[ntype]
            ...
            z = skip_gate_fc1(hm) + skip_gate_fc2(h0_dst) # 
            z_m = torch.sigmoid(z)
            z = skip_gate_fc1(torch.zeros_like(hm)) + skip_gate_fc2(h0_dst) # 
            z0 = torch.sigmoid(z)
            z_out[ntype] = z_m - z0
        return z_out
...
class SelfAttentionReducer(nn.Module):
    def __init__(self,
        ...
        for dsttype in self.dst_ntypes:
            if use_norm:
                self.norms[dsttype] = nn.LayerNorm(out_dim)
    ...
    def hetero_atten(self, trans_out):
        # Self-attention
        trans_out_original = trans_out
        # [N, 2, h]
        q = self.q_linear(trans_out).view(-1, 2, self.num_heads, self.d_k).unsqueeze(2)
        # [N, 2, 1, head, dk]
        k = self.k_linear(trans_out).view(-1, 2, self.num_heads, self.d_k).unsqueeze(1)
        # [N, 1, 2, head, dk]
        v = self.v_linear(trans_out).view(-1, 2, self.num_heads, self.d_k).unsqueeze(1)
        # [N, 1, 2, head, dk]
        attn_score = (q * k).sum(-1) / self.sqrt_dk
        attn_score = F.softmax(attn_score, dim=2).unsqueeze(-1)
        # [N, 2, 2, head, 1]
        trans_out = (attn_score * v).sum(2)
        # [N, 2, head, dk]
        trans_out = trans_out.reshape(-1, 2, self.out_dim)
        trans_out = trans_out_original + trans_out
        # [N, 2, h]
    ...
    def forward(self, mfg, trans_outs, h0):
        ...
        for ntype, trans_out in trans_outs.items():
            if trans_out.shape[1] == 1:
                ...
                trans_out = self.hetero_atten(trans_out)
            if self.use_norm:
                h_new[ntype] = self.norms[ntype](trans_out)
            else:
                h_new[ntype] = trans_out
        return dict(h_new=h_new)
...
class MeanReducer(nn.Module):
    ...
    def forward(self, mfg, trans_outs, h0):
        ...
        for ntype, trans_out in trans_outs.items():
            trans_out = trans_out.mean(1)
            if self.use_norm:
                trans_out = self.norms[ntype](trans_out)
            h_new[ntype] = trans_out
        return dict(h_new=h_new)


@REDUCER.register_module()
class LMFReducer(nn.Module):
    def __init__(self,
                 relations,
                 hidden_dim,
                 rank,
        ...
        if 'HasTag' in self.etypes and 'SubTopic' in self.etypes:
            self.factor_1 = nn.Parameter(torch.Tensor(rank, hidden_dim + 1, hidden_dim))
            self.factor_2 = nn.Parameter(torch.Tensor(rank, hidden_dim + 1, hidden_dim))
            nn.init.xavier_normal_(self.factor_1)
            nn.init.xavier_normal_(self.factor_2)
    ...
    def lmf(self, trans_out):
        # [N, 2, d]
        # trans_out1 = self.dropout(trans_out[:,0,:])
        # trans_out2 = self.dropout(trans_out[:,1,:])
        trans_out1 = trans_out[:,0,:]
        trans_out2 = trans_out[:,1,:]
        trans_out1 = F.pad(trans_out1, [0, 1], value=1)
        trans_out2 = F.pad(trans_out2, [0, 1], value=1)
        # [N, d+1]
        trans_out1 = torch.matmul(trans_out1, self.factor_1)
        trans_out2 = torch.matmul(trans_out2, self.factor_2)
        # [r, N, d]
        trans_out = torch.stack([trans_out1, trans_out2], dim=3)
        # [r, N, d, 2]
        trans_out = torch.prod(trans_out, dim=3)
        # [r, N, d]
        trans_out = trans_out.sum(0)
        # [N, d]
        return trans_out
...
class CommonUniqueReducer(nn.Module):
    def __init__(self,
        ...
        if 'HasTag' in self.etypes and 'SubTopic' in self.etypes:
            ...
            # self.fusionu1 = nn.Linear(hidden_dim, hidden_dim)
            # self.fusionu2 = nn.Linear(hidden_dim, hidden_dim)
    ...
    def attentive_fusion(self, common1, common2):
        atten_score1 = self.atten1(common1)
        atten_score2 = self.atten2(common2)
        common = atten_score1*common1 + atten_score2*common2
        return common
    ...
    def common_unique(self, trans_out):
        ...
        trans_out = (self.attentive_fusion(common1, common2) + unique.max(1).values) / 2
        # [N, d]
        return trans_out, unique, common

    def intra_ntype_reduce(self, trans_out, ntype):
        """
            Aggregate messages.
        """
        if ntype == 'video':
            ...
            # ('tag', 'HasVideo', 'video')
            trans_out = trans_out.mean(1, keepdim=True)
        else:
            # Possible messages:
            # ('tag', 'SubTopic', 'tag')
            # ('tag', 'SuperTopic', 'tag')
            # ('video', 'HasTag', 'tag')
            tag_trans_out = [] # messages from tags
            video_trans_out = [] # messages from videos
            idx = 0
            for srctype, etype, dsttype in self.relations:
                if dsttype != 'tag':
                    continue
                if srctype == 'tag':
                    tag_trans_out.append(trans_out[:,idx])
                else:
                    video_trans_out.append(trans_out[:,idx])
                idx += 1
            assert len(video_trans_out) == 1
            trans_out = [video_trans_out[0]]
            if len(tag_trans_out):
                tag_trans_out = sum(tag_trans_out) / len(tag_trans_out)
            if len(tag_trans_out):
                trans_out.append(tag_trans_out)
    ...
    def forward(self, mfg, trans_outs, h0):
        ...
        h_common = {}
        for ntype, trans_out in trans_outs.items():
            trans_out = self.intra_ntype_reduce(trans_out, ntype)
            if trans_out.shape[1] == 1:
                # Only one inbound edge etype
                trans_out = trans_out.mean(1)
            else:
    ...
    def forward(self, mfg, trans_outs, h0):
        ...
        for ntype, trans_out in trans_outs.items():
            if trans_out.shape[1] == 1:
                ...
                trans_out = self.common_unique(trans_out)
            h_new[ntype] = self.norms[ntype](trans_out)
