...
from multimodal.utils import import_module_error_func
...
def build_reactor(cfg):
    """Build reactor."""
    return REACTORS.build(cfg)
...
def build_recommender(cfg):
    """Build Recommender."""
    return RECOMMENDERS.build(cfg)


def build_gnn(cfg):
    """Build GNN."""
    return GNNS.build(cfg)


def build_linker(cfg):
    """Build Linker."""
    return LINKERS.build(cfg)
...
def build_model(cfg, train_cfg=None, test_cfg=None):
    ...
    if obj_type in RECOMMENDERS:
        return build_recommender(cfg)
    ...
    raise ValueError(f'{obj_type} is not registered in '
                     'LOCALIZERS, RECOGNIZERS, RECOMMENDER or DETECTORS')