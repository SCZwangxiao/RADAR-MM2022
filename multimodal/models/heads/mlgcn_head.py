...
class MLGCNHead(BaseHead):
    ...
    def forward(self, x, num_segs):
        ...
        # [B, 1, in_channels]
        tag_feats = self.tag_fc(self.tag_feats+self.tag_feats_id)
        # [num_classes, in_channels]
        tag_graph = self.tag_graph.to(tag_feats.device)
        for gnn, norm in zip(self.tag_gnns, self.gnn_norm):
            ...
            # [num_classes, in_channels]
        tag_feats = tag_feats.unsqueeze(0)
        # [1, num_classes, in_channels]
        if self.dropout is not None:
            x = self.dropout(x)
        cls_score = (x * tag_feats).sum(2)
        # [B, num_classes]
        return cls_score
